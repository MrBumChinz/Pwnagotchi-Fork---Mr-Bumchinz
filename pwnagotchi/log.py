...
# Filter for session-relevant messages only (what pwnagotchi AI needs to parse)
class SessionFilter(logging.Filter):
    SESSION_PATTERNS = [
        ...
        'connecting to http',
        'deauthing ',
        'sending association frame',
        '!!! captured new handshake',
        'detected unit ',
    ]
    def filter(self, record):
        msg = record.getMessage()
        return any(p in msg for p in self.SESSION_PATTERNS)
...
class LastSession(object):
    ...
    def _parse_stats(self):
        ...
        self.duration = '%02d:%02d:%02d:%02d' % (days, hours, mins, secs)
        self.duration_human = []
        if hours > 0:
...
def setup_logging(args, config):
    ...
    logger.setLevel(logging.DEBUG)  # Always DEBUG - handlers filter
    ...
    if filename:
        ...
        # File handler for session data only (what pwnagotchi parses for persistence)
    file_handler = logging.FileHandler(filename)
    file_handler.setLevel(logging.DEBUG)  # Level doesn't matter, filter does
    file_handler.setFormatter(formatter)
    file_handler.addFilter(SessionFilter())  # Only session-relevant messages
