...
/* UI State - Static allocation, no malloc in hot paths */
...
static void init_ui_state(void) {
    ...
    strncpy(g_ui_state.aps, "0 (00)", sizeof(g_ui_state.aps) - 1);
    strncpy(g_ui_state.uptime, "00:00:00", sizeof(g_ui_state.uptime) - 1);
    strncpy(g_ui_state.shakes, "0 (00)", sizeof(g_ui_state.shakes) - 1);
...
static int handle_command(const char *cmd, char *response, size_t resp_size) {
    ...
    /* SET_BLUETOOTH status - Plugin: BT-Tether */
...
");
    ...
    /* SET_MEMTEMP_HEADER header - Plugin: Memtemp */
...
");
    ...
    /* SET_MEMTEMP_DATA data - Plugin: Memtemp */
...
int main(int argc, char *argv[]) {
    ...
    const char *display_type = "waveshare2in13_v2";
    int server_fd = -1;
