...
static void init_ui_state(void) {
    ...
    strncpy(g_ui_state.uptime, "00:00:00", sizeof(g_ui_state.uptime) - 1);
...
static int handle_command(const char *cmd, char *response, size_t resp_size) {
    ...
    if (strcmp(cmd_name, "UPDATE") == 0) {
        if (g_dirty) {
            ...
            if (now - g_last_update_ms >= UPDATE_INTERVAL_MS) {
                ...
                display_partial_update(g_framebuffer, 0, 0, 0, 0);  /* Partial - no blink */
    ...
    /* LIST_THEMES - Get list of available themes */
    if (strcmp(cmd_name, "LIST_THEMES") == 0) {
        int count = themes_count();
        if (count <= 0) {
            snprintf(response, resp_size, "OK No themes available\n");
        } else {
            char *p = response;
    ...
    /* GET_THEME - Get current active theme name */
    if (strcmp(cmd_name, "GET_THEME") == 0) {
        const char *current = theme_get_active();
        if (current && current[0]) {
            snprintf(response, resp_size, "OK %s\n", current);
        } else {
            snprintf(response, resp_size, "OK (default)\n");
        }
        return 0;
    }
    
    /* DISABLE_THEME - Disable themes and use default ASCII faces */
    if (strcmp(cmd_name, "DISABLE_THEME") == 0) {
        themes_disable();
        g_dirty = 1;
        snprintf(response, resp_size, "OK Themes disabled\n");
        PWNAUI_LOG_INFO("Themes disabled, using default ASCII faces");
        return 0;
    }
...
int main(int argc, char *argv[]) {
    ...
    if (g_native_plugins) {
        ...
        if (plugins_init(&g_plugins) < 0) {
            ...
            PWNAUI_LOG_INFO("GPS: enabled=%d fd=%d", g_plugins.gps_enabled, g_plugins.gps.udp_fd);
        }
    }
    ...
    while (g_running) {
        ...
        if (g_dirty) {
            ...
            if (now - g_last_update_ms >= UPDATE_INTERVAL_MS) {
                renderer_render_ui(&g_ui_state, g_framebuffer);
                display_partial_update(g_framebuffer, 0, 0, 0, 0);
                g_last_update_ms = now;
                g_dirty = 0;
            }
        }
