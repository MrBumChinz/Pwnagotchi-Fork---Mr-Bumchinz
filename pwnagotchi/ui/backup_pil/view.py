...
# PwnaUI integration
try:
    from pwnagotchi.ui import pwnaui_patch as pwnaui
    _pwnaui_client = pwnaui.get_client() if pwnaui.is_available() else None
except:
    _pwnaui_client = None
...
class View(object):
    def __init__(self, config, impl, state=None):
        ...
        self._state = State(state={
            ...
            'uptime': LabeledValue(color=BLACK, label='UP', value='00:00:00:00', position=self._layout['uptime'],
                                   label_font=fonts.Bold,
                                   text_font=fonts.Medium),
    ...
    def set(self, key, value):
        ...
        if _pwnaui_client:
            try:
                if key == 'face': _pwnaui_client.set_face(value)
                elif key == 'status': _pwnaui_client.set_status(str(value).replace(chr(10), ' ')[:80])
                elif key == 'name': _pwnaui_client.set_name(value)
                elif key == 'channel': _pwnaui_client.set_channel(value)
                elif key == 'aps': _pwnaui_client.set_aps(value)
                elif key == 'uptime': _pwnaui_client.set_uptime(value)
                elif key == 'shakes': _pwnaui_client.set_shakes(value)
                elif key == 'mode': _pwnaui_client.set_mode(value)
                elif key == 'friend_name': _pwnaui_client.set_friend(value if value else '')
            except: pass
    ...
    def update(self, force=False, new_data={}):
        ...
        self.set('uptime', '%02d:%02d:%02d:%02d' % (days, hours, mins, secs))
        ...
        with self._lock:
            ...
            if force or len(changes):
                ...
                self._state.reset()
                # Trigger pwnaui display update
                if _pwnaui_client:
                    try:
                        _pwnaui_client.update() if not force else _pwnaui_client.full_update()
                    except: pass
