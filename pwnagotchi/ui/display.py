...
from pwnaui_view import PwnaUIView as View, DummyHardwareImpl

# Try to import hardware, but don't fail if missing
try:
    import pwnagotchi.ui.hw as hw
    HW_AVAILABLE = True
except ImportError:
    HW_AVAILABLE = False
    logging.warning("Hardware display modules not available")


def get_display_impl(config):
    """Get display implementation, with fallback to dummy."""
    if not HW_AVAILABLE:
        return DummyHardwareImpl(config)
    
    try:
        return hw.display_for(config)
    except (ImportError, ModuleNotFoundError) as e:
        logging.warning(f"Display driver not found: {e}, using dummy impl")
        return DummyHardwareImpl(config)
    except Exception as e:
        logging.warning(f"Error loading display driver: {e}, using dummy impl")
        return DummyHardwareImpl(config)


class Display(View):
    def __init__(self, config, state={}):
        impl = get_display_impl(config)
        super(Display, self).__init__(config, impl, state)
    ...
    def _on_view_rendered(self, img):
        ...
        if self._enabled and img is not None:
