...
class PwnaUIClient:
    ...
    def _close_socket(self):
        """Properly close socket to avoid leaking file descriptors"""
        if self._socket:
            try:
                self._socket.close()
            except:
    ...
    def connect(self):
        ...
        # Clean up any old socket before creating new one
        self._close_socket()
            
        try:
            self._socket = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            self._socket.settimeout(2.0)  # Increased timeout
            self._socket.connect(self.SOCKET_PATH)
            self._connected = True
            return True
        except Exception as e:
            logging.debug(f"[pwnaui] Connect failed: {e}")
            self._close_socket()  # Clean up on failure
            return False
    ...
    def send(self, cmd):
        ...
        try:
            ...
            # Timeout is NOT an error - socket is still valid
            # Don't close it, just return None
            logging.debug(f"[pwnaui] Timeout on command: {cmd[:20]}...")
            return None
        except Exception as e:
            # Real socket error - close and mark for reconnect
            logging.debug(f"[pwnaui] Socket error: {e}")
            self._close_socket()
            return None
