...
def get_plugin_metadata(filepath):
    """Extract plugin metadata from a .py file without importing it"""
    metadata = {
        'description': None,
        'author': None,
        'version': None,
    ...
    try:
        with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read(4096)  # Read first 4KB for metadata
            
            # Extract __description__
            ...
            # Extract __help__
            match = re.search(r"__help__\s*=\s*['\"](.+?)['\"]", content)
            if match:
                metadata['help'] = match.group(1)
                
            # Try docstring as fallback for description
...
class Handler:
    ...
    def plugins(self, name, subpath):
        if name is None:
            ...
            plugin_metadata = {}
            for plugin_name, filepath in plugins.database.items():
                plugin_metadata[plugin_name] = get_plugin_metadata(filepath)
            return render_template('plugins.html', loaded=plugins.loaded, database=plugins.database, plugin_metadata=plugin_metadata)

        if name == 'toggle' and request.method == 'POST':
            try:
                checked = True if 'enabled' in request.form else False
                plugin_name = request.form['plugin']
                logging.info(f"[WebUI] Toggling plugin {plugin_name} to {checked}")
                result = plugins.toggle_plugin(plugin_name, checked)
                logging.info(f"[WebUI] Toggle result: {result}")
                return 'success' if result else 'failed'
            except Exception as e:
                logging.exception(f"[WebUI] Error toggling plugin: {e}")
        ...
        if name in plugins.loaded and plugins.loaded[name] is not None and hasattr(plugins.loaded[name], 'on_webhook'):
            ...
            if name == 'themes':
                return render_template('status.html', 
                    title=pwnagotchi.name(),
                    go_back_after=0,
                    message='🎨 The themes plugin needs to be enabled first! Go to Plugins and enable "themes".')
            abort(404)
