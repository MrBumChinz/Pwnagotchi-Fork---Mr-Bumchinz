...
class PwnaUIView:
    ...
    def __init__(self, config, impl=None, state=None):
        ...
        if PWNAUI_AVAILABLE:
            ...
            if self._pwnaui.connect():
                logging.debug("Connected to PwnaUI daemon")
    ...
    def on_wifi(self, aps):
        ...
        if self._agent:
            ...
            # Only show current APs count (total goes to TAPS via SET_STATS)
            self.set('aps', str(current))
            if self._pwnaui and self._pwnaui.connected:
                self._pwnaui.set_aps(str(current))
                # Send total to TAPS: SET_STATS pwds fhs phs taps
                # We only know taps here, other stats come from pwnhub_stats
                self._pwnaui.send(f'SET_STATS 0 0 0 {total}')
        self.update()
