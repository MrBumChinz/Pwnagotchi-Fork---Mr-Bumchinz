...
void renderer_render_ui(ui_state_t *state, uint8_t *framebuffer) {
    ...
    /* Bottom row: PWND | MODE | BT ✓/✗ | GPS ✓/✗ */
    draw_labeled_value(state, framebuffer, L->shakes_x, L->shakes_y, "PWND", state->shakes);
    
    /* Mode: AUTO/MANU */
    renderer_draw_text(state, framebuffer, L->mode_x, L->mode_y, state->mode, FONT_BOLD);
