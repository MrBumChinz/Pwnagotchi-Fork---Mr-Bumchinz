            ...
            for key in changes:
                ...
                if key == 'face':
                    ...
                    if isinstance(elem, LabeledValue):
                        ...
                        value = str(elem)
                    # Convert uptime to compact format for daemon display
                    # From 'D: XX H: XX M: XX S: XX' to 'DD:HH:MM:SS'
                    if value:
                        import re
                        match = re.match(r'D:\s*(\d+)\s*H:\s*(\d+)\s*M:\s*(\d+)\s*S:\s*(\d+)', value)
                        if match:
                            d, h, m, s = match.groups()
                            value = f'{int(d):02d}:{int(h):02d}:{int(m):02d}:{int(s):02d}'
                    ...
                    value = elem if isinstance(elem, str) else getattr(elem, 'value', str(elem))
                    # Convert mode to full text for display
                    if value == 'AUTO':
                        value = 'Auto Mode'
                    elif value == 'MANU':
                        value = 'Manual Mode'
