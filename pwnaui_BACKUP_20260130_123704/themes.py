    ...
    def _load_voice(self):
        ...
        try:
            ...
            if hasattr(self.voice_module, "Voice"):
                ...
                log.info(f"Loaded voice.py from {self.theme_dir}")
...
class Theme:
    ...
    def _load(self):
        ...
        log.info(f"Theme '{self.name}' loaded: {len(self.face_images)} faces from {self.faces_dir}")
...
class ThemeManager:
    ...
    def _discover_themes(self):
        ...
        log.info(f"Discovered {len(self.themes)} themes")
    ...
    def set_active_theme(self, name: str) -> bool:
        ...
        log.info(f"Active theme set to: {name}")
        return True
...
class FaceRenderer:
    ...
    FACE_MAX_WIDTH = 80   # Max width for face
    FACE_MAX_HEIGHT = 80  # Max height for face
    FACE_X = 85           # X position (centered in display)
